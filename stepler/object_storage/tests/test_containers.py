...
def test_download_object(container_steps):
    ...
    **Teardown:**
    ...
    container_name, file_name, content, object_name = utils.generate_ids(
        count=4)
    container_steps.create(name=container_name)
    created_object_name = list(utils.generate_files(prefix='test'))[0]
    container_steps.put_object(container_name=container_name,
                               object_name=created_object_name,
                               content=content)
    downloaded_object_name = container_steps.get_object_from_container(
        container_name=container_name, object_name=created_object_name,
        file_name=created_object_name)
    container_steps.check_object_hash(created_object_name,
                                      downloaded_object_name)
