...
class ImagesSteps(BaseSteps):
    ...
    def add_metadata(self, image_name, metadata, check=True):
        ...
        if check:
            page_images.table_images.row(
                name=image_name,
                status=config.STATUS_ACTIVE).wait_for_presence()
    ...
    def check_image_info(self,
        ...
        if expected_description:
            assert_that(self.app.page_image.image_info_main.description.value,
                        equal_to(expected_description))
        else:
            description = self.app.page_image.image_info_main.description
            assert_that(description.is_present, equal_to(False))
