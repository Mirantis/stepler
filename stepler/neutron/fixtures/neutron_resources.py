...
def neutron_2_servers_2_nets_diff_tenants(
        ...
        get_security_group_steps, get_server_steps, create_floating_ip):
    ...
    for i in range(2):
        ...
        create_floating_ip(public_network, port=server_port,
                           project_id=project_id)

        # Wait for real floating ip attach. Otherwise AssertionError (empty
        # collection) occurs in test in get_floating_ip(server)
        waiter.wait(lambda: server_steps.get_floating_ip(server),
                    timeout_seconds=config.SERVER_ATTACH_IP_TIMEOUT,
