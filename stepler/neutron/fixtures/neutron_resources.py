...
def neutron_2_servers_2_nets_diff_tenants(
        ...
        public_network, cirros_image, public_flavor, port_steps,
        get_security_group_steps, get_server_steps, create_floating_ip, get_nova_floating_ip_steps):
...
@pytest.fixture
def neutron_2_servers_2_tenants_with_shared_net(request, port_steps,create_floating_ip,
        role_steps, create_project, create_user, create_network, create_subnet,
        create_router, router_steps, public_network, add_router_interfaces,
        get_server_steps, conntrack_cirros_image, public_flavor, get_security_group_steps, get_nova_floating_ip_steps, nova_create_floating_ip):
    base_name, = utils.generate_ids()
    resources = []
    final_resources = []
    admin_role = role_steps.get_role(name=config.ROLE_ADMIN)
