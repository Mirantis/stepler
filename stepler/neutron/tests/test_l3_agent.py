...
pytestmark = pytest.mark.requires("nova_api_count_gte(3)",
...
def test_ban_one_l3_agent(cirros_image,
    ...
    **Steps:**
    ...
    #. Get node with l3 agent for router1
    ...
    **Teardown:**
    ...
    #. Delete flavor
    """
    network_1, network_2 = neutron_2_servers_diff_nets_2_routers.networks
    router_1, _ = neutron_2_servers_diff_nets_2_routers.routers
    servers = neutron_2_servers_diff_nets_2_routers.servers
    ...
    for _ in range(ban_count):
        ...
        node_fqdn = agent_steps.get_hosts_with_l3_agent_for_router(
            router=router_1)
        nodes_with_l3 = os_faults_steps.get_nodes(fqdns=node_fqdn)

        os_faults_steps.terminate_service(config.NEUTRON_L3_SERVICE,
                                          nodes=nodes_with_l3)
        agent_steps.check_router_rescheduled(
            router_1, old_l3_agent,
            timeout=config.L3_AGENT_RESCHEDULING_TIMEOUT)

    server_3 = server_steps.create_servers(
        image=cirros_image,
        flavor=flavor,
        networks=[network_1],
    ...
    server_1, server_2 = neutron_2_servers_diff_nets_2_routers.servers
    server_steps.check_ping_between_servers_via_floating(
        (server_1, server_3,),
        ip_types=('fixed',),
        timeout=config.PING_BETWEEN_SERVERS_TIMEOUT)
