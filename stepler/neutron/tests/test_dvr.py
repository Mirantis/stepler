...
def test_north_south_connectivity_after_ban_clear_l3_on_compute(
    ...
    **Teardown:**
    ...
    computes = os_faults_steps.get_node(
        fqdns=[getattr(server, config.SERVER_HOST_ATTR)])
    os_faults_steps.terminate_service(config.NEUTRON_L3_SERVICE, computes)
    os_faults_steps.start_service(config.NEUTRON_L3_SERVICE, computes)
