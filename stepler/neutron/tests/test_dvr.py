...
def test_check_east_west_connectivity_instances_on_different_hosts(
        neutron_2_servers_different_networks,
        nova_floating_ip,
        server_steps):
    """**Scenario:** Check east-west connectivity between instances on
    different hosts.
...
@pytest.mark.idempotent_id('564c2d1e-3871-48d7-a9c5-199d5f273c2a')
@pytest.mark.parametrize('neutron_2_servers_different_networks',
                         ['same_host'],
                         indirect=True)
@pytest.mark.parametrize('router', [dict(distributed=True)], indirect=True)
def test_check_east_west_connectivity_instances_on_the_same_host(
        neutron_2_servers_different_networks,
        nova_floating_ip,
        server_steps):
    """**Scenario:** Check east-west connectivity between instances on
    ...
    **Setup:**
    ...
    #. Create network_2 with subnet_2
    #. Add network_2 interface to router
    #. Create server_2 on the same compute and connect it to network_2
