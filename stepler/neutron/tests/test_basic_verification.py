...
def test_neutron_security_groups_quota(current_project,
    ...
    **Teardown:**
    ...
    # Calculate count of security groups to be created to reach max count.
    # Currently 'default' security group is available only => 4 groups to be
    # created. 'Skip' can be added here in case of any failure in future:
...
def test_neutron_sec_group_rules_quota(current_project,
    ...
    **Teardown:**
    ...
    rules_count = len(neutron_security_group_rule_steps.get_rules(
        tenant_id=current_project.id))
    # Calculate count of security group rules to be created to reach max count.
    # Currently 'default' security group contains 4 rules, newly created group
    # contains 2 rules => 4 rules to be created to reach max count.
    # 'Skip' can be added here in case of any failure (if count_to_create < 0)
    # or quota can be increase in case of changes for default count of rules
    # per group (default and newly created)
