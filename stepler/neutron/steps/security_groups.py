...
class NeutronSecurityGroupSteps(base.BaseSteps):
    ...
    def get_security_groups(self, check=True, **kwargs):
        ...
        if check:
            err_msg = ("No groups are retrieved which correspond "
                       "requested parameters {!r}").format(kwargs)
            assert_that(groups, is_not(empty()), err_msg)
            if kwargs:
                for group in groups:
                    err_msg = ("Group with ID {!r} doesn't correspond "
                               "requested parameters").format(group['id'])
                    assert_that(group, has_entries(kwargs), err_msg)
