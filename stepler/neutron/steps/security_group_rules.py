...
class NeutronSecurityGroupRuleSteps(base.BaseSteps):
    ...
    def get_rules(self, check=True, **kwargs):
        ...
        if check:
            err_msg = ("No rules are retrieved which correspond "
                       "requested parameters {!r}").format(kwargs)
            assert_that(rules, is_not(empty()), err_msg)
            if kwargs:
                for rule in rules:
                    err_msg = ("Rule with ID {!r} doesn't correspond "
                               "requested parameters").format(rule['id'])
                    assert_that(rule, has_entries(kwargs), err_msg)
    ...
    def check_negative_create_extra_group_rule(self, group_id, **rule_params):
        ...
        assert_that(
            calling(self.add_rule_to_group).with_args(group_id,
                                                      check=False,
                                                      **rule_params),
            raises(exceptions.OverQuotaClient, exception_message),
            "Security group rule for group with ID {!r} has been created "
            "though it exceeds the quota or OverQuotaClient exception with "
            "expected error message has not been appeared".format(group_id))
