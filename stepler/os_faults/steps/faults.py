...
class OsFaultsSteps(base.BaseSteps):
    ...
    @steps_checker.step
    def check_metrics(self, start_time):
        """Step to check appearance of new metrics.

        Metrics are got from influx databases on all monitoring nodes.
        ...
        for fqdn in [node.fqdn for node in mon_nodes]:
            ...
            if stdout:
                break
        assert_that(stdout, is_not(empty()))
