...
def test_dispatch_external_event(cirros_image,
                                 flavor,
                                 security_group,
                                 create_network,
                                 create_subnet,
    ...
    **Teardown:**
        ...
        #. Delete cirros image
    """
    network = create_network('net01')
    subnet = next(utils.generate_ids('subnet'))
    create_subnet(subnet, network=network, cidr='192.168.1.0/24')
    ...
    str_to_find = 'server_external_events.*status.*completed' + \
