...
def skip_live_migration_tests(request, nova_ceph_enabled):
    """Skip tests with wrong `block_migartion` parameter.
    ...
    if not hasattr(request.node, 'callspec'):
        return
    block_migartion = request.node.callspec.params.get('block_migration')
    if block_migartion is None:
        return
    if block_migartion and nova_ceph_enabled:
