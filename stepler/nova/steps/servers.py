...
class ServerSteps(base.BaseSteps):
    ...
    @steps_checker.step
    def attach_fixed_ip(self, server, network_id, check=True, timeout=0):
        ...
        if check:
            ...
            waiter.wait(_check_fixed_ip_attached, timeout_seconds=timeout)

            ips_after_attach = self.get_ips(server, 'fixed').keys()
            return (set(ips_after_attach) - set(ips_before_attach)).pop()

    @steps_checker.step
    def detach_fixed_ip(self, server, fixed_ip=None, check=True, timeout=0):
        ...
        if check:
            ...
            waiter.wait(_check_detach_fixed_ip, timeout_seconds=timeout)
