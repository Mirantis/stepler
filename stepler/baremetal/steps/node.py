...
class IronicNodeSteps(BaseSteps):
    ...
    def check_ironic_node_maintenance(self,
        ...
        def predicate():
            try:
                get_node = self._client.node.get(node_id=node.uuid)
                assert_that(get_node.maintenance, equal_to(state))
                is_maintenance_changed = True
            except exceptions.NotFound:
                is_maintenance_changed = False

            return expect_that(is_maintenance_changed,
