...
def get_ironic_chassis_steps(ironic_client):
    ...
    def _get_ironic_chassis_steps(**credentials):
        return steps.IronicChassisSteps(ironic_client(**credentials).chassis)

    return _get_ironic_chassis_steps


@pytest.yield_fixture
def ironic_chassis_steps(unexpected_chassis_cleanup, get_ironic_chassis_steps, cleanup_chassis):
    ...
    chassis_before = _chassis_steps.get_chassis(check=False)
...
@pytest.fixture(scope='session')
def cleanup_chassis():
    def _cleanup_chassis(_chassis_steps, limit=0, uncleanable_chassis=None):
        ...
        if len(deleting_chassis) > limit:
            ...
            for chs in deleting_chassis:
                _chassis_steps.delete_ironic_chassis(chs)

    return _cleanup_chassis


@pytest.fixture(scope='session')
def primary_chassis(get_ironic_chassis_steps, cleanup_chassis):
    ...
    """
    chassis_before = set()
    for chs in get_ironic_chassis_steps.get_chassis(check=False):
