...
def pytest_collection_modifyitems(session, items):
    ...
    for item in items:
        ...
        test_platforms = [marker.args[0] for marker in markers]
        skip_msg = 'Test is only for platform(s): ' + ', '.join(test_platforms)

        for test_platform in test_platforms:
            if test_platform == specified_platform:
                break
        else:
...
for _name in PLATFORMS:
    platform[_name] = pytest.mark.platform(_name)
del _name
