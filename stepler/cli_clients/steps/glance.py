...
class CliGlanceSteps(base.BaseCliSteps):
    ...
    def create_image(self, image_file, disk_format, container_format,
        ...
        Returns:
            str: result of command shell execution (exit_code, stdout, stderr)

        Raises:
            AnsibleExecutionException: if command execution is failed
        """
        cmd = 'glance image-create'
        if image_file:
            cmd += ' --file {0}'.format(image_file)
        if image_name:
            cmd += ' --name {0}'.format(image_name)
        if disk_format:
            cmd += ' --disk-format {0}'.format(disk_format)
        if container_format:
            cmd += ' --container-format {0}'.format(container_format)
    ...
    def check_negative_image_create_without_properties(self, filename,
        ...
        exit_code, stdout, stderr = self.create_image(image_file=filename,
                                                      disk_format=None,
