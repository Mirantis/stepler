...
@pytest.mark.idempotent_id('30537390-e1ba-47bc-aca0-db94a740f728',
                           api_version=2)
@pytest.mark.parametrize('api_version', [1, 2])
def test_project_in_image_member_list(cirros_image, project, cli_glance_steps,
                                      glance_steps, api_version):
    """**Scenario:** Verify 'glance member-list' command.

    Test checks that 'glance member-list --image_id <id>' show binded project
...
@pytest.mark.idempotent_id('3adc71e3-2362-4a95-aac6-93a14da65487',
                           api_version=2)
@pytest.mark.parametrize('api_version', [1, 2])
def test_create_image_member(cirros_image, project, cli_glance_steps,
                             glance_steps, api_version):
...
@pytest.mark.idempotent_id('e2393652-ae09-42ae-87fd-7b16adeaa6f7',
                           api_version=2)
@pytest.mark.parametrize('api_version', [1, 2])
def test_delete_image_member(cirros_image, project, cli_glance_steps,
                             glance_steps, api_version):
    ...
    **Teardown:**
    ...
    cli_glance_steps.delete_image_member(cirros_image, project,
                                         api_version=api_version)
    glance_steps.check_image_bind_status(cirros_image, project, binded=False)
