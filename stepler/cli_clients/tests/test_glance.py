...
def test_status_manipulation_via_locations(patch_ini_file_and_restart_services,
                                           glance_steps,
                                           cli_glance_steps):
    """**Scenatio:** Glance image status manipulation through locations removal.

    **Steps:**

    #. Create glance image
    #. Set `show_multiple_locations=True` in /etc/glance/glance-api.conf and
       restart glance service.
    #. Add 2 urls to image locations
    #. Check that user cant manipulate of image status through locations removal
    """
    image = glance_steps.create_images(
