...
def test_change_image_status_directly(cirros_image, auth_steps, glance_steps):
    """**Scenario:** Cerify that user can't change image status directly with
        v1 API.
    ...
    **Steps:**
        ...
        #. Get token
        #. Send PUT request to flance image endpoint with
            {'x-image-meta-status': 'queued'} headers
        #. Check that images status is still active
