...
def test_delete_volume_cascade(volume_steps, snapshot_steps):
    ...
    **Steps:**
    ...
    volume = volume_steps.create_volumes([volume_name])
    snapshot_name = next(utils.generate_ids(prefix='snapshot'))
    snapshot_steps.create_snapshots(volume[0], [snapshot_name])
    volume_steps.delete_volumes(volume, cascade=True)


@pytest.mark.idempotent_id('fbcb9a58-9a4d-4864-88db-c08a14475994')
def test_negative_delete_volume_cascade(volume, volume_snapshot, volume_steps):
