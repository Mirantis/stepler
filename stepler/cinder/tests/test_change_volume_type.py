...
def test_change_volume_type_from_empty(create_volume_type, create_volume,
                                       volume_steps, volume_types_steps):
    ...
    **Teardown:**
    ...
    volume_type_name = next(utils.generate_ids(prefix='volume_type'))
    volume_type = create_volume_type(volume_type_name)

    volume_steps.volume_retype(volume, volume_type, 'never')


@pytest.mark.idempotent_id('a5cee93e-6d58-420e-a61c-85cbd10d6725')
def test_change_volume_type(create_volume_type, create_volume,
                            volume_steps, volume_types_steps):
    ...
    **Teardown:**
    ...
    #. Delete volume types
    """
    volume_type_name_1 = next(utils.generate_ids(prefix='volume_type'))
    volume_type_name_2 = next(utils.generate_ids(prefix='volume_type'))
    ...
    volume_steps.volume_retype(volume, volume_type_2, 'never')
