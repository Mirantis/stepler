...
def test_restart_all_cinder_services(cirros_image,
                                     volume,
                                     server,
                                     nova_floating_ip,
    ...
    **Teardown:**
    ...
    volume_1 = volume_steps.create_volumes(image=cirros_image)[0]
    attach_volume_to_server(server, volume_1, device='/dev/vdc')

    server_steps.execute_commands(server,
    ...
    volume_2 = volume_steps.create_volumes(image=cirros_image)[0]
    attach_volume_to_server(server, volume_2, device='/dev/vdd')

    server_steps.execute_commands(
        server, ['sudo mkdir /mnt/volume_2',
