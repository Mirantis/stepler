...
class ApiClientV2(BaseApiClient):
    ...
    def volumes_create(self,
        ...
        return base.Resource(self, json.loads(response.json())['volume'])
    ...
    def volumes_list(self, volume_id):
        """List volumes."""
        response = self._get('/volumes/' + volume_id)

        volumes = []
        for volume in json.loads(response.json())['volumes']:
            volumes.append(base.Resource(self, volume))
        return volumes
