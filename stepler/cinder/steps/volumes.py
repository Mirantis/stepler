...
class VolumeSteps(base.BaseSteps):
    ...
    @steps_checker.step
    def check_negative_volume_not_created_non_exist_type(self, image):
        """Step for negative volume creation with non-existed volume type.
        ...
        Raises:
            AssertionError: if check triggered an error
        """
        type_name = next(utils.generate_ids(prefix='volume_type'))
    ...
    @steps_checker.step
    def volume_retype(self, volume, volume_type, policy, check=True):
