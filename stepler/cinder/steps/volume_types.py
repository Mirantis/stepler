...
from hamcrest import assert_that, is_in, is_not, empty  # noqa
import waiting

from stepler import base
from stepler.third_party import steps_checker

__all__ = ['VolumeTypesSteps']
...
class VolumeTypesSteps(base.BaseSteps):
    ...
    def delete_volume_type(self, volume_type, check=True):
        ...
        if check:
            self.check_volume_type_presence(volume_type, present=False)

    @steps_checker.step
    def check_volume_type_presence(self, volume_type, present=True, timeout=0):
        """Check step volume type presence status.

        Args:
            volume_type (object): cinder volume type to check presence status
            present (bool|True): flag whether volume type should present or not
